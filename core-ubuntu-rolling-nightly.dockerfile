...
RUN apt update -y && apt install -y curl sudo ca-certificates  --quiet --no-install-recommends
...
RUN sh -c "$(curl -fsSL https://gcore.jsdelivr.net/gh/HYwooo/install@master/install-zsh.sh)"
