...
RUN apt update -y  && apt install -y curl ca-certificate sudo --quiet --no-install-recommends
...
RUN sed -i 's/^ZSH_THEME=.*/ZSH_THEME="materialshell\/materialshell"/' ~/.zshrc && \
    ...
    echo "alias /bin/sh=/usr/bin/zsh /usr/bin/sh=/usr/bin/zsh" >> ~/.zshrc && alias /bin/sh=/usr/bin/zsh /usr/bin/sh=/usr/bin/zsh
